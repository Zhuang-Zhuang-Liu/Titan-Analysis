...
sys.stdout.reconfigure(line_buffering=True)   # 设置stdout为非缓冲模式，确保实时输出
os.environ['PYTHONUNBUFFERED'] = '1'  # 设置环境变量PYTHONUNBUFFERED
...
initial_message = os.getenv("CAMEL_TASK", "默认任务：请统计最近这个季度的订单数据")
...
model = ModelFactory.create(
    ...
    api_key=api_key,       
    url=api_url, 
    model_config_dict={ "temperature": 1,"max_tokens": 4000}
)
